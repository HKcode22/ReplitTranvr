...
| **2. Webhook ingress** (`POST /api/v1/webhooks/aerodatabox/:secret`) | ⏳ **STUB VERIFIED** (acks + logs only) | Endpoint returns 2xx fast and now bypasses CSRF (routes registered before CSRF middleware). Confirm real deliveries via the app log `[adb-v3-webhook] received flights=N ...`. Phase 2 full validator (`flightStatus_v3.ts`) + Phase 3 extractor/store not built yet — rows are NOT written yet. |
