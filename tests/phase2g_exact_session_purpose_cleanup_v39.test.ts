...
describe("Phase2G exact prepaid-session purpose cleanup helper", () => {
  it("is scoped to exact session blob/runtime cleanup and refuses while provider exposure remains", () => {
    ...
    expect(src).toContain("listSubscriptionsStrict");
    expect(src).toContain("REFUSED:ACTIVE_BILLABLE_SUBSCRIPTIONS");
  ...
  it("records zero provider/subscription/credit mutation claims", () => {
    ...
    expect(src).toContain("active_billable_subscriptions:0");
  });
});
