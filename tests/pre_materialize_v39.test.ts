...
describe("materializePreSnapshotsForCutoff", () => {
  it("builds one snapshot per eligible horizon and persists idempotently", async () => {
    const inserted: unknown[][] = [];
    const result = await materializePreSnapshotsForCutoff(
      fakePool([row()], (p) => inserted.push(p)),
      { cutoffUtc: CUTOFF, batchId: null, frameHash: "f", configHash: "c" },
    );
  ...
  it("opposite-movement rows never snapshot; null schedule blocks with reason", async () => {
    const result = await materializePreSnapshotsForCutoff(
      ...
      { cutoffUtc: CUTOFF, batchId: null, frameHash: null, configHash: null },
    );
  ...
  it("population SELECT failure throws (fail-closed, no partial run)", async () => {
    ...
    await expect(
      materializePreSnapshotsForCutoff(badPool, { cutoffUtc: CUTOFF, batchId: null, frameHash: null, configHash: null }),
  ...
  it("horizon subset restricts output", async () => {
    const result = await materializePreSnapshotsForCutoff(
      fakePool([row()]),
      { cutoffUtc: CUTOFF, batchId: null, horizons: ["T-90m"], frameHash: null, configHash: null },
    );
