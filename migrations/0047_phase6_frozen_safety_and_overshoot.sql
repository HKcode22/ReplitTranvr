...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='adb_phase6_authorization_safety_values' AND conrelid='clean.adb_phase6_authorization'::regclass) THEN
    ALTER TABLE clean.adb_phase6_authorization ADD CONSTRAINT adb_phase6_authorization_safety_values CHECK (
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='adb_phase6_authorization_base_cap_1900' AND conrelid='clean.adb_phase6_authorization'::regclass) THEN
    ALTER TABLE clean.adb_phase6_authorization ADD CONSTRAINT adb_phase6_authorization_base_cap_1900 CHECK (alert_cap_per_parent_day = 1900) NOT VALID;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='adb_phase6_soft_margin_covers_unsettled' AND conrelid='clean.adb_phase6_authorization'::regclass) THEN
    ALTER TABLE clean.adb_phase6_authorization ADD CONSTRAINT adb_phase6_soft_margin_covers_unsettled CHECK (
      daily_soft_stop_margin_credits IS NULL OR daily_soft_stop_margin_credits >= unsettled_burst_margin_credits
    );
  END IF;
END $$;

ALTER TABLE clean.adb_incident_stop DROP CONSTRAINT IF EXISTS adb_incident_stop_cause_check_v2;
ALTER TABLE clean.adb_incident_stop DROP CONSTRAINT IF EXISTS adb_incident_stop_cause_check_v3;
ALTER TABLE clean.adb_incident_stop ADD CONSTRAINT adb_incident_stop_cause_check_v3
  CHECK (cause IN ('authentication','raw-persistence','persistence','reconciliation','deletion','segment_activation'));
...
CREATE TRIGGER trg_phase6_settlement_evidence_immutable BEFORE UPDATE OR DELETE ON clean.adb_phase6_settlement_evidence FOR EACH ROW EXECUTE FUNCTION clean.reject_phase6_settlement_evidence_mutation();

CREATE OR REPLACE FUNCTION clean.require_phase6_settlement_evidence() RETURNS trigger LANGUAGE plpgsql AS $$
...
CREATE TABLE IF NOT EXISTS clean.adb_budget_day_adjustment (
  ...
  CHECK ((source_run_day_index=31 AND target_run_day_index IS NULL) OR (source_run_day_index<31 AND target_run_day_index=source_run_day_index+1))
);
CREATE UNIQUE INDEX IF NOT EXISTS uq_budget_adjustment_target ON clean.adb_budget_day_adjustment(target_run_day_index) WHERE target_run_day_index IS NOT NULL;

CREATE OR REPLACE FUNCTION clean.apply_phase6_budget_limits_before_insert() RETURNS trigger LANGUAGE plpgsql AS $$
...
CREATE OR REPLACE FUNCTION clean.mark_phase6_hard_cap_mismatch() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION clean.record_phase6_hard_cap_overshoot() RETURNS trigger LANGUAGE plpgsql AS $$
...
-- Any persistent incident that appears while a Phase-6 parent exists is also
-- mirrored into the batch-scoped delivery-failure signal. The runtime owner
-- already treats that signal as an immediate stop/settle condition. This makes
-- raw-persistence/authentication/global persistence incidents stop an already
-- active segment instead of merely blocking the next start.
CREATE OR REPLACE FUNCTION clean.propagate_incident_to_phase6_failure() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_propagate_incident_to_phase6_failure ON clean.adb_incident_stop;
