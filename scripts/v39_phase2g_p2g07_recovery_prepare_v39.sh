...
AMENDMENT="artifacts/phase2g-compact6-p2g07-provider502-recovery-freeze-20260922.json"
RUNTIME="artifacts/phase2g-gate2-runtime-P2G-S1-20260922-07.json"
BUDGET="P2G-S1-20260922-07"
AUTH="SEPmd/V3.9_PHASE2G_AUTH_20260922_P2G08.json"
AUTH_ID="AUTH-20260922-P2G08"
...
case "${1:-help}" in
  static)
    ...
    npx vitest run \
      ...
      tests/phase2g_probe_guard_band_v39.test.ts \
      tests/prepaid_probe_runtime_v39.test.ts
    npx tsc --noEmit
    echo "RECOVERY_STATIC=PASS"
    ;;
  runtime)
    require_repo_state
    ...
    echo "RECOVERY_RUNTIME_SHA=$(sha256sum "$RUNTIME" | awk '{print $1}')"
    echo "RECOVERY_AMENDMENT_SHA=$(sha256sum "$AMENDMENT" | awk '{print $1}')"
    ;;
  auth-draft)
    require_repo_state
    ...
    npx tsx scripts/v39_prepare_phase2g_auth_v39.ts \
      ...
      --smoke-runtime-sha "$SMOKE_RUNTIME_SHA" \
      --preprobe "$PREPROBE"
    echo "RECOVERY_AUTH_SHA=$(sha256sum "$AUTH" | awk '{print $1}')"
    ;;
  auth-approve)
    require_repo_state
  ...
  preflight)
    ...
    out="artifacts/phase2g-stage1-paid-preflight-P2G08-${stamp}.json"
  ...
  *)
    echo "Usage:"
    echo "  bash scripts/v39_phase2g_p2g07_recovery_prepare_v39.sh static"
    echo "  bash scripts/v39_phase2g_p2g07_recovery_prepare_v39.sh runtime"
    echo "  bash scripts/v39_phase2g_p2g07_recovery_prepare_v39.sh auth-draft"
    echo "  PHASE2G_CONFIRM_AUTH_SHA=<sha> bash scripts/v39_phase2g_p2g07_recovery_prepare_v39.sh auth-approve"
    echo "  bash scripts/v39_phase2g_p2g07_recovery_prepare_v39.sh preflight"
    ;;
esac
