...
function main(): void {
  ...
  if (!ledger.includes(handoff.evidenceId)) {
    appendFileSync(LEDGER, [
      ...
      `- ingress_binding_kind: ${handoff.smoke.ingressBindingKind}`,
      `- ingress_binding_evidence_id: ${handoff.smoke.ingressBindingEvidenceId}`,
      `- INGRESS_BINDING_ARTIFACT_SHA256:${handoff.smoke.ingressBindingArtifactSha256}`,
      `- INGRESS_BINDING_FILE_SHA256:${handoff.smoke.ingressBindingFileSha256}`,
      `- INGRESS_BINDING_SHA256:${handoff.smoke.ingressBindingSha256}`,
      `- ingress_origin: ${handoff.smoke.ingressOrigin}`,
