...
async function main(): Promise<void> {
  ...
  if (open.rowCount !== 2 || open.rows.map((r) => String(r.id)).join(",") !== "1,2") {
    throw new Error(
      `REFUSED:OPEN_INCIDENT_SET_CHANGED:${open.rows.map((r) => `${r.id}:${r.cause}`).join(",")}`,
    );
  }

  const one = open.rows[0];
  const two = open.rows[1];
  ...
  const basisSha256 = sha256(fs.readFileSync(basisPath));
  ...
  try {
    ...
    if (
      locked.rowCount !== 2 ||
      locked.rows.some((r) => r.resolved !== false)
    ) {
    ...
    if (otherOpen.rowCount !== 0) {
      throw new Error(
        `REFUSED:NEW_OPEN_INCIDENT_DURING_RECOVERY:${otherOpen.rows[0].id}:${otherOpen.rows[0].cause}`,
      );
    }
