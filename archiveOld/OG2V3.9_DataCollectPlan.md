...
and feature availability are separate dimensions, NOT prerequisites for snapshot
...
**Aircraft/tail identity (§46, CORRECTED):** Priority fallback: `aircraft_reg (registration, e.g. 9V-SKD)` > `aircraft_mode_s` > `icao24` > `provider aircraft ID`. `tail_known = (registration NOT NULL)`. `missing_tail` → chain break, not linked. `conflicting identifiers` (reg vs mode_s disagree) → `tail_conflict_flag=true`, not linked. `aircraft_swap` (tail changes mid-rotation) → chain break at swap. `tail_chain_links` counted only on consistent registration across legs within `max_turnaround=6h` — do NOT require same calendar date (cross-midnight rotations like 23:50→00:50 are legitimate). Next leg's origin must equal previous leg's operational/scheduled destination.
...
5. **Settle:** sleep a documented window, read `B_after`, then `B_after_2`;
 ...
 9. **PASS iff** `|C_external − C_internal| = 0` AND failures = 0
...
| ---- | ---- | ---- |
| `successor_scheduled` | Tail has a chronological next scheduled departure from same airport within `max_turnaround=6h`, next leg's `origin = previous destination` (cross-midnight rotations are legitimate) | YES (denominator) |
