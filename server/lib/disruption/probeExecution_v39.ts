 ...
 * Binding constants: Stage1=2h, Stage2=4h, capacity=60 rows/hour,
 * cumulative immutable probe-budget-day hard cap=500 Alert credits.
 *
 * Once a provider subscription id exists, every terminal path attempts DELETE
 * before returning. Runtime/account uncertainty never becomes an ordinary
 * completed probe; it is settled when possible and otherwise leaves durable
 * MISMATCH + incident evidence.
 */
...
export interface ProbeExecutionDependencies {
  ...
  listSubscriptions: typeof listSubscriptionsStrict;
...
const defaultDeps: ProbeExecutionDependencies = {
  ...
  createSubscription,
  deleteSubscription,
  listSubscriptions: listSubscriptionsStrict,
  checkAirportFeeds,
};
...
export function loadProbeRuntimeConfig(
...
): { config: ProbeRuntimeConfig; sha256: string } {
  assertHash(expectedSha256, "probe runtime artifact");
  const raw = readFileSync(path, "utf8");
  const actual = hash(raw);
  if (actual !== expectedSha256.toLowerCase()) {
    throw new Error(`REFUSED: probe runtime artifact hash mismatch expected=${expectedSha256} actual=${actual}`);
  }
  const x = JSON.parse(raw);
  const config: ProbeRuntimeConfig = {
    version: String(x.version ?? ""),
    probeBudgetDayId: String(x.probeBudgetDayId ?? ""),
    watchdogPollMs: positiveInt(x.watchdogPollMs, "watchdogPollMs"),
    minStabilityBuckets: positiveInt(x.minStabilityBuckets, "minStabilityBuckets"),
    settlementInitialWaitSeconds: nonnegativeInt(x.settlementInitialWaitSeconds, "settlementInitialWaitSeconds"),
    settlementPollIntervalSeconds: positiveInt(x.settlementPollIntervalSeconds, "settlementPollIntervalSeconds"),
    settlementStableReadCount: positiveInt(x.settlementStableReadCount, "settlementStableReadCount"),
    settlementTimeoutSeconds: positiveInt(x.settlementTimeoutSeconds, "settlementTimeoutSeconds"),
  ...
  if (
    config.stage1ReservationCredits > PROBE_BUDGET_DAY_HARD_CAP ||
    config.stage2ReservationCredits > PROBE_BUDGET_DAY_HARD_CAP
  ) {
...
export function loadProbeExecutionArtifacts(input: {
  ...
  for (const [label, hour] of [
    ["stage1UtcSlotHour", tc.stage1UtcSlotHour],
    ["stage2UtcSlotHour", tc.stage2UtcSlotHour],
  ] as const) {
    if (!Number.isInteger(hour) || hour < 0 || hour > 23) throw new Error(`REFUSED: invalid ${label}`);
  }
  if (
    !["weekday", "weekend"].includes(tc.stage1WeekdayClass) ||
    !["weekday", "weekend"].includes(tc.stage2WeekdayClass)
  ) {
  ...
  return {
    preprobe: ext as FrozenProbeArtifact & ProbeArtifactExecutionExtension,
    preprobeSha256: frozen.artifactHash,
...
export function assertProbeTimeClass(stage: 1 | 2, now: Date, config: ProbeTimeClassConfig): void {
  ...
  if (weekdayClass(now) !== targetWeekday) {
    throw new Error(`REFUSED_TIME_CLASS: stage ${stage} requires ${targetWeekday}`);
  }
...
export function requireFrozenCandidate(
  ...
  if (!candidate) {
    throw new Error(
      `REFUSED_MEMBERSHIP: ${normalized} is not an authorized ${allowReplacement ? "shortlist/replacement" : "Stage-1 shortlist"} candidate`,
    );
  }
  return candidate;
}
...
async function openIncident(kind: string, detail: unknown): Promise<void> {
  await pool.query(
    `INSERT INTO clean.adb_incident_stop(cause,occurred_at_utc,detail,resolved)
     VALUES('reconciliation',now(),$1::jsonb,false)`,
    [JSON.stringify({ kind, owner: "anchor_probe", ...((detail && typeof detail === "object") ? detail as object : { detail }) })],
  );
}

async function assertR1Clean(deps: ProbeExecutionDependencies): Promise<void> {
  const subs = await deps.listSubscriptions(); // strict reader throws on unknown account state
  const foreign = subs.filter((s) => s.isActive && s.billingType !== "LifetimeBased");
  if (foreign.length) {
    throw new Error(
      `REFUSED_R1: ${foreign.length} active billable subscription(s): ${foreign.map((x) => x.id).join(",")}`,
    );
  }
}
...
export async function ensureProbeBudgetDay(config: ProbeRuntimeConfig): Promise<void> {
  const mismatch = await pool.query(
    `SELECT probe_budget_day_id FROM clean.adb_probe_budget_day WHERE state='MISMATCH' LIMIT 1`,
  );
  if (mismatch.rowCount) {
    throw new Error(`REFUSED_PROBE_BUDGET_MISMATCH: ${mismatch.rows[0].probe_budget_day_id}`);
  }
  const open = await pool.query(
    `SELECT probe_budget_day_id FROM clean.adb_probe_budget_day WHERE state='OPEN'`,
  );
  if (open.rowCount && open.rows[0].probe_budget_day_id !== config.probeBudgetDayId) {
    throw new Error(
      `REFUSED_PROBE_BUDGET_OPEN: ${open.rows[0].probe_budget_day_id} must settle/close before ${config.probeBudgetDayId}`,
    );
  }
  await pool.query(
  ...
  if (current.rows[0]?.state !== "OPEN") {
    throw new Error(
      `REFUSED_PROBE_BUDGET_STATE: ${config.probeBudgetDayId} is ${current.rows[0]?.state ?? "missing"}`,
    );
  }
}

/**
 * Budget-day exposure. For a live probe, reservation is used until actual
 * external/internal evidence exceeds it. Optional exclusion is used by the
 * active probe so prior exposure is not approximated by subtracting a reserve.
 */
export async function probeBudgetExposure(dayId: string, excludeProbeId: number | null = null): Promise<number> {
  const r = await pool.query(
...
async function reserveProbe(
  candidate: FrozenProbeCandidate,
  stage: 1 | 2,
  now: Date,
  runtime: ProbeRuntimeConfig,
  preprobeHash: string,
  ...
  try {
    ...
    if (
      Number(exposure.rows[0]?.n ?? 0) + reservation + runtime.unsettledBurstMarginCredits >
    ...
    if (now.toISOString().slice(0, 10) !== end.toISOString().slice(0, 10)) {
      throw new Error(
        "REFUSED_PROBE_MIDNIGHT: probe may not cross UTC midnight without an explicit split budget identity",
      );
    }
...
async function exposureSnapshot(
  ...
  const r = await pool.query(
    `SELECT COALESCE(sum(COALESCE(delivery_attempt_cost_credits,adb_cost_credits,notification_items,0)),0)::int internal
       FROM clean.raw_delivery WHERE subscription_id=$1`,
    [subscriptionId],
  );
  const b = await deps.getBalance();
  return {
    internal: Number(r.rows[0]?.internal ?? 0),
    external: balanceBefore !== null && b ? balanceBefore - b.creditsRemaining : null,
...
function completeBucketStability(
  ...
  minBuckets: number,
): { stability: number | null; buckets: number } {
  const width = 15 * 60_000;
  const first = Math.ceil(start.getTime() / width) * width;
  const last = Math.floor(end.getTime() / width) * width;
  const counts: number[] = [];
  for (let t = first; t + width <= last; t += width) {
    counts.push(firstObservationMs.filter((x) => x >= t && x < t + width).length);
  }
...
async function computeProbeMetrics(
  subscriptionId: string,
  start: Date,
  end: Date,
  creditsSpent: number,
  ...
  const ids = await pool.query(
    `SELECT wir.resolution_status,wir.flight_instance_id
  ...
  const confirmedIds = new Set(
    ids.rows
      .filter((x: any) => x.resolution_status === "resolved" && x.flight_instance_id)
      .map((x: any) => String(x.flight_instance_id)),
  );
  ...
  const stability = completeBucketStability(
    ...
    firstObs.rows.map((r: any) => Number(r.event_ms)),
    runtime.minStabilityBuckets,
  );
  const hours = Math.max((end.getTime() - start.getTime()) / 3_600_000, 1 / 3600);
  const rowsDelivered = Number(raw.rows[0]?.rows ?? 0);
  const links = Number(chain.rows[0]?.links ?? 0);
  return {
    rowsDelivered,
    confirmedUniqueLower: lower,
    confirmedPlusAmbiguousUpper: upper,
    tailChainLinks: links,
    rowsPerHour: rowsDelivered / hours,
    ...
    completeBuckets: stability.buckets,
    stabilityStatus: stability.stability === null ? "INSUFFICIENT_SAMPLE" : "PASS",
  };
}
...
async function markProbeFailure(input: {
  probeId: number;
  dayId: string;
  ended: Date;
  stopReason: string;
  ...
  settlementReads?: number | null;
  incidentKind: string;
  incidentDetail?: Record<string, unknown>;
}): Promise<void> {
  await pool.query(
    `UPDATE clean.adb_anchor_probe
        SET status='failed',window_end=$2,balance_after=COALESCE($3,balance_after),
            credits_spent=COALESCE($4,credits_spent),internal_send_credits=COALESCE($5,internal_send_credits),
            settlement_reads=COALESCE($6,settlement_reads),settlement_stable_balance=COALESCE($3,settlement_stable_balance),
            stop_reason=$7
      WHERE probe_id=$1`,
    [
      input.probeId,
      input.ended,
      input.stableBalance ?? null,
      input.creditsSpent ?? null,
      input.internal ?? null,
      input.settlementReads ?? null,
      input.stopReason,
    ],
  );
  await pool.query(
    `UPDATE clean.adb_probe_budget_day SET state='MISMATCH',closed_at=COALESCE(closed_at,now())
      WHERE probe_budget_day_id=$1`,
    [input.dayId],
  );
  await openIncident(input.incidentKind, {
    probeId: input.probeId,
    probeBudgetDayId: input.dayId,
    stopReason: input.stopReason,
    ...(input.incidentDetail ?? {}),
  });
}

export async function executeProbe(
  input: ExecuteProbeInput,
  deps: ProbeExecutionDependencies = defaultDeps,
  ...
  if (input.authMaxAlertCredits <= 0 || reservation > input.authMaxAlertCredits) {
    throw new Error("REFUSED_AUTH_CEILING: probe reservation exceeds AUTH Alert ceiling");
  }
  const before = await deps.getBalance();
  if (!before) throw new Error("REFUSED_BALANCE_UNKNOWN");
  if (before.creditsRemaining < 1000 + reservation + runtime.unsettledBurstMarginCredits) {
    throw new Error("REFUSED_BALANCE_HEADROOM");
  }

  const probeId = await reserveProbe(candidate, input.stage, started, runtime, input.artifacts.preprobeSha256);
  const sub = await deps.createSubscription("FlightByAirportIcao", candidate.icao, { maxDeliveryRetries: 0 });
  if (!sub?.id) {
    // Migration 0046 also opens a reconciliation incident from this transition.
    await pool.query(
      `UPDATE clean.adb_anchor_probe SET status='failed',stop_reason='subscription_create_failed' WHERE probe_id=$1`,
      [probeId],
    );
    return {
      probeId,
  ...
  try {
    await pool.query(
      `UPDATE clean.adb_anchor_probe SET subscription_id=$1,balance_before=$2,window_start=$3 WHERE probe_id=$4`,
      [sub.id, before.creditsRemaining, started, probeId],
    );
  } catch (error: any) {
    const deleted = await deps.deleteSubscription(sub.id).catch(() => false);
    await markProbeFailure({
      probeId,
      dayId: runtime.probeBudgetDayId,
      ended: deps.now(),
      stopReason: deleted ? "probe_persistence_failure" : "provider_delete_failure",
      incidentKind: deleted ? "probe_persistence_failure" : "provider_delete_failure",
      incidentDetail: { subscriptionId: sub.id, error: error?.message ?? String(error), deleted },
    }).catch(() => undefined);
    throw error;
  }
  ...
  try {
    while (deps.now() < targetEnd) {
      await deps.sleep(
        Math.min(runtime.watchdogPollMs, Math.max(1, targetEnd.getTime() - deps.now().getTime())),
      );
      ...
      try {
        ...
        runtimeError = error;
        stopReason = "incident_stop";
        break;
      }

      const live = await exposureSnapshot(sub.id, before.creditsRemaining, deps);
      if (live.currentBalance === null || live.external === null || live.external < 0) {
        stopReason = "authoritative_balance_unavailable";
        break;
      }
      const prior = await probeBudgetExposure(runtime.probeBudgetDayId, probeId);
      const active = Math.max(live.internal, live.external);
      if (prior + active + runtime.unsettledBurstMarginCredits >= PROBE_BUDGET_DAY_HARD_CAP) {
        stopReason = "probe_cap_soft_stop";
        break;
      }
      if (active + runtime.unsettledBurstMarginCredits >= reservation) {
        stopReason = "probe_reservation_soft_stop";
        break;
      }
    }
  } catch (error) {
    runtimeError = error;
    stopReason = stopReason ?? "probe_runtime_failure";
  }
  ...
  if (!deleted) {
    await markProbeFailure({
      ...
      dayId: runtime.probeBudgetDayId,
      ended,
      stopReason: "provider_delete_failure",
      incidentKind: "provider_delete_failure",
      incidentDetail: { subscriptionId: sub.id, runtimeError: runtimeError instanceof Error ? runtimeError.message : runtimeError },
    });
    return {
      probeId,
      status: "failed",
  ...
  const settlementConfig: SettlementConfig = {
    initialWaitSeconds: runtime.settlementInitialWaitSeconds,
    pollIntervalSeconds: runtime.settlementPollIntervalSeconds,
    stableReadCount: runtime.settlementStableReadCount,
    timeoutSeconds: runtime.settlementTimeoutSeconds,
  };
  const settled = await runSettlement(
    settlementConfig,
    async () => (await deps.getBalance())?.creditsRemaining ?? null,
  );
  if (settled.status !== "settled") {
    await markProbeFailure({
      ...
      dayId: runtime.probeBudgetDayId,
      ended,
      stopReason: "SETTLEMENT_UNRESOLVED",
      settlementReads: settled.readsUsed,
      incidentKind: "settlement_unresolved",
      incidentDetail: { reason: settled.reason },
    });
    return {
      probeId,
      status: "failed",
  ...
  const creditsSpent = before.creditsRemaining - settled.stableBalance;
  const exposure = await exposureSnapshot(sub.id, before.creditsRemaining, deps);
  if (creditsSpent < 0 || exposure.internal !== creditsSpent) {
    await markProbeFailure({
      probeId,
      dayId: runtime.probeBudgetDayId,
      ended,
      stopReason: "RECONCILIATION_MISMATCH",
      stableBalance: settled.stableBalance,
      creditsSpent: creditsSpent >= 0 ? creditsSpent : null,
      internal: exposure.internal,
      settlementReads: settled.readsUsed,
      incidentKind: "reconciliation_mismatch",
      incidentDetail: { creditsSpent, internal: exposure.internal },
    });
    return {
      probeId,
      status: "failed",
  ...
  if (runtimeError !== null) {
    await markProbeFailure({
      ...
      internal: exposure.internal,
      settlementReads: settled.readsUsed,
      incidentKind: "probe_runtime_failure",
      incidentDetail: {
  ...
  const metrics = await computeProbeMetrics(sub.id, started, ended, creditsSpent, runtime);
  const durationCensored = ended.getTime() < targetEnd.getTime() - 1000;
  const finalStop = durationCensored ? (stopReason ?? "safety_stop") : null;
  await pool.query(
    `UPDATE clean.adb_anchor_probe SET status='completed',window_end=$2,window_hours=$3,
      balance_after=$4,credits_spent=$5,internal_send_credits=$6,rows_delivered=$7,
      unique_flights=$8,tail_chain_links=$9,rows_per_hour=$10,unique_flights_per_credit=$11,
      tail_chain_links_per_credit=$12,stability=$13,duration_censored=$14,stop_reason=$15,
      complete_buckets=$16,min_stability_buckets=$17,confirmed_unique_lower=$8,
      confirmed_plus_ambiguous_upper=$18,settlement_reads=$19,settlement_stable_balance=$4,
      stability_status=$20 WHERE probe_id=$1`,
    [
      probeId,
      ended,
      (ended.getTime() - started.getTime()) / 3_600_000,
      settled.stableBalance,
      creditsSpent,
      exposure.internal,
      metrics.rowsDelivered,
      metrics.confirmedUniqueLower,
      metrics.tailChainLinks,
      metrics.rowsPerHour,
      metrics.uniqueFlightsPerCredit,
      metrics.tailChainLinksPerCredit,
      metrics.stability,
      durationCensored,
      finalStop,
      metrics.completeBuckets,
      runtime.minStabilityBuckets,
      metrics.confirmedPlusAmbiguousUpper,
      settled.readsUsed,
      metrics.stabilityStatus,
    ],
  );

  const dayExposure = await probeBudgetExposure(runtime.probeBudgetDayId);
  if (dayExposure > PROBE_BUDGET_DAY_HARD_CAP) {
    ...
    await openIncident("probe_cap_overshoot", {
      probeBudgetDayId: runtime.probeBudgetDayId,
      dayExposure,
    });
...
export async function closeProbeBudgetDay(dayId: string): Promise<void> {
  ...
  const probing = await pool.query(
    `SELECT count(*)::int n FROM clean.adb_anchor_probe WHERE probe_budget_day_id=$1 AND status='probing'`,
    [dayId],
  );
  if (Number(probing.rows[0]?.n ?? 0) > 0) {
    throw new Error("REFUSED: cannot close probe budget day with active probing attempt");
  }
  const unresolved = await pool.query(
    `SELECT count(*)::int n FROM clean.adb_anchor_probe
      WHERE probe_budget_day_id=$1 AND status='failed'
        AND (credits_spent IS NULL OR internal_send_credits IS NULL)`,
    [dayId],
  );
  if (Number(unresolved.rows[0]?.n ?? 0) > 0) {
    throw new Error("REFUSED: failed probe has unresolved spend");
  }
