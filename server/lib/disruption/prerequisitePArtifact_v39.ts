...
export const PREREQUISITE_P_SCHEMA_VERSION = "v3.9-prepaid-security-retention-1" as const;
...
export function verifyPrerequisitePArtifact(
  value: unknown,
  expected: { codeSha?: string; retentionMatrixSha256?: string } = {},
  ...
  if (expected.codeSha && artifact.code_sha?.toLowerCase() !== expected.codeSha.toLowerCase()) failures.push("code-sha-mismatch");
...
export function readCurrentPrerequisitePArtifact(root = process.cwd(), codeSha = currentGitSha(root), retentionMatrixSha256?: string): { pass: boolean; failures: string[]; artifact: PrerequisitePArtifact | null } {
  ...
  try {
    ...
    const verdict = verifyPrerequisitePArtifact(artifact, { codeSha, retentionMatrixSha256 });
    return { ...verdict, artifact: verdict.pass ? artifact : null };
  } catch {
    return { pass: false, failures: ["artifact-unreadable"], artifact: null };
  }
}
