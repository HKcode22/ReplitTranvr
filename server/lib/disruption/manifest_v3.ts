...
/**
 * Source inventory entries never self-certify tests/evidence. Current closure
 * tooling must supply evidence tied to the exact current bytes/SHA.
 */
const current = (
  ...
  description: string,
  requirements: string[] = [],
): ManifestEntry => ({ type, path, description, implemented: true, tested: false, evidenceId: null, requirements });

export const V39_MANIFEST: ManifestEntry[] = [
  current("module", "server/lib/disruption/adbCollectionController_v3.ts", "collection admission/provider-mutation owner"),
  current("module", "server/lib/disruption/phase6SamplingDecision_v39.ts", "frozen Phase-6 slot/region/adaptive decision owner", ["REQ-ADAPT-001", "REQ-CAL-001"]),
  current("module", "server/lib/disruption/adaptiveMi_v3.ts", "binding REGIONAL adaptation", ["REQ-ADAPT-001", "REQ-ADAPT-002", "REQ-ADAPT-003", "REQ-ADAPT-004"]),
  current("module", "server/lib/disruption/aerodataboxLimiter_v3.ts", "central provider transport/REST ledger", ["REQ-RATE-001"]),
  current("module", "server/lib/disruption/fidsCensus_v3.ts", "provider-observable FIDS population and fail-closed identity", ["REQ-FIDS-001", "REQ-FIDS-002", "REQ-FIDS-003"]),
  ...
  current("module", "server/lib/disruption/rawIngress_v3.ts", "raw-before-2xx immutable ingress", ["REQ-RAW-001", "REQ-RAW-002"]),
  ...
  current("module", "server/lib/disruption/outcomeTerminalizer_v3.ts", "target terminalizer"),
  current("module", "server/lib/disruption/outcomeRecoveryOrchestrator_v39.ts", "bounded original-query outcome recovery"),
  current("module", "server/lib/disruption/historicalFeatureStore_v3.ts", "bitemporal history fail-closed store", ["REQ-HIST-001", "REQ-HIST-002", "REQ-HIST-003"]),
  ...
  current("module", "server/lib/disruption/anchorPromotion_v39.ts", "exact-five Stage-2 promotion"),
  ...
  current("module", "server/lib/disruption/settlement_v3.ts", "stable balance settlement", ["REQ-SETTLE-001"]),
  current("module", "server/lib/disruption/gates_v3.ts", "gate computation"),
  current("module", "server/lib/disruption/retentionSecurity_v39.ts", "security/retention machinery"),
  current("module", "server/lib/disruption/authRecord_v39.ts", "authorization record verifier"),
  ...
  current("module", "server/db.ts", "boot migration enforcement"),
  current("script", "scripts/v39_phase6_start_owner_v39.ts", "Phase-6 start owner/anti-bypass boundary"),
  current("script", "scripts/v39_probe_stage2_v39.ts", "AUTH Stage-2 wrapper"),
  current("script", "scripts/v39_probe_stage2_owner_v39.ts", "exact-five Stage-2 owner"),
  current("script", "scripts/anchor_probe.ts", "Stage-1/2 probe execution and budget-day lifecycle"),
  current("script", "scripts/calendar_solve.ts", "strict calendar CLI"),
  current("script", "scripts/v39_security_verify_v39.ts", "deployment-aware security verifier"),
  current("script", "scripts/v39_preflight_v39.ts", "aggregate Phase-0 preflight"),
  current("script", "scripts/db_verify_phase0_v39.ts", "migration/schema verification"),
  current("test", "tests/adaptation.test.ts", "binding adaptation tests"),
  current("test", "tests/timestamps_raw.test.ts", "timestamp/raw tests"),
  current("test", "tests/weather_history.test.ts", "weather/history and history-infrastructure tests"),
  current("test", "tests/fids_identity_failclosed_v39.test.ts", "FIDS service-date/identity fail-closed tests"),
  current("test", "tests/provider_fids.test.ts", "provider/FIDS contract tests"),
  current("test", "tests/pre_materialize_v39.test.ts", "PRE materialization tests"),
  current("test", "tests/airborne_materializer_v39.test.ts", "AIRBORNE materializer tests"),
  current("test", "tests/airborne_pipeline_v39.test.ts", "AIRBORNE builder/pipeline tests"),
  current("test", "tests/outcome_recovery_v39.test.ts", "outcome recovery tests"),
  current("test", "tests/flight_identity_v39.test.ts", "canonical identity/retime tests"),
  current("test", "tests/anchor_probe.test.ts", "probe safety tests"),
  current("test", "tests/anchor_promotion_v39.test.ts", "Stage-2 promotion tests"),
  current("test", "tests/calendar_solver_v39.test.ts", "calendar compatibility tests"),
  current("test", "tests/calendar_solver_strict_v39.test.ts", "strict calendar hard-constraint tests"),
  current("test", "tests/fidsTimezone.test.ts", "FIDS timezone/DST tests"),
  current("test", "tests/runtime_safety_v39.test.ts", "runtime fail-closed tests"),
  current("test", "tests/budget_accounting_v39.test.ts", "budget/accounting tests"),
  current("test", "tests/gates_v39.test.ts", "gate/settlement tests"),
  current("test", "tests/retention_security_v39.test.ts", "retention/security tests"),
  current("test", "tests/scanner_manifest_v39.test.ts", "manifest/scanner tests"),
  current("test", "tests/phase0_closure_v39.test.ts", "Phase-0 closure tests"),
  ...
  current("migration", "migrations/0033_incident_stop.sql", "persistent incident stop"),
  ...
  current("migration", "migrations/0037_phase6_sampling_decision_state.sql", "frozen slot/region and adaptive-state provenance"),
  current("migration", "migrations/0038_phase6_parent_segment_lifecycle.sql", "parent/segment/gap collection lifecycle"),
  current("document", "SEPmd/V3.9_DataCollectPlan_f.8.md", "binding Plan §§0-21"),
  current("document", "SEPmd/V3.9_IMPLEMENTATION_LOG.md", "implementation manual"),
...
export function splitPartitionForDay(i: number): EvalPartition | null {
  ...
  return i <= 20 ? "TRAIN" : i <= 25 ? "VALIDATION" : "TEST";
}
function sha(s: string) { return createHash("sha256").update(s).digest("hex"); }
export function splitRuleHash(seed: string) {
  return sha(JSON.stringify({
    train: [1, 20], validation: [21, 25], test: [26, 31], gap: 0,
    groupBy: "flight_instance_id", seed, ordering: "run_day_index_chronological",
  }));
}
...
export function checkManifestCompleteness(
  ...
  return { complete: !missingEvidence.length && !missingImplementation.length, missingEvidence, missingImplementation };
}
...
export function deriveManifestHashProof(root = process.cwd(), entries: ManifestEntry[] = V39_MANIFEST): ManifestHashProof {
  ...
  for (const e of entries.filter((x) => !["document", "config"].includes(x.type))) {
    try {
      ...
      artifacts.push({ path: e.path, sha256: createHash("sha256").update(bytes).digest("hex"), exists: true });
    } catch {
...
export function checkManifestHashProof(proof: ManifestHashProof, entries: ManifestEntry[] = V39_MANIFEST) {
  const failures: string[] = [];
  if (proof.missing.length) failures.push(...proof.missing.map((x) => `missing:${x}`));
  const required = new Set(entries.filter((x) => !["document", "config"].includes(x.type)).map((x) => x.path));
  for (const p of proof.artifacts) {
    if (required.has(p.path) && (!p.exists || !/^[a-f0-9]{64}$/.test(p.sha256))) failures.push(`invalid-hash:${p.path}`);
  }
  for (const path of required) if (!proof.artifacts.some((p) => p.path === path)) failures.push(`unproved:${path}`);
