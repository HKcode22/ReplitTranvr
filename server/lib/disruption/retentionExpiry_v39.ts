...
export const DEFAULT_MAX_RAW_RETENTION_DAYS = 7;
...
export function validateRetentionDays(raw: string | number | undefined): number {
  const n = Number(raw);
  if (!Number.isInteger(n) || n < 1 || n > DEFAULT_MAX_RAW_RETENTION_DAYS) {
...
function expiryFromAge(age: string, days: number): string {
  return new Date(new Date(age).getTime() + days * 86_400_000).toISOString();
}

function assertApplyEvidence(): void {
  ...
  let deployment: any;
  try { deployment = JSON.parse(deploymentRaw); } catch { throw new Error("V39_RETENTION_DEPLOYMENT_EVIDENCE_NOT_JSON"); }
  for (const surface of ["primary", "replica", "backup", "object", "log"]) {
    const state = deployment?.surfaces?.[surface];
    if (!state || state === "UNKNOWN") throw new Error(`RETENTION_SURFACE_UNVERIFIED:${surface}`);
  }
  if (deployment.surfaces.primary !== "DEPLOYED") throw new Error("PRIMARY_RETENTION_SURFACE_MUST_BE_DEPLOYED");
}
...
export async function collectRetentionExpiryCandidates(
  now = new Date(),
  retentionDays = validateRetentionDays(process.env.V39_RAW_PROVIDER_RETENTION_DAYS),
  ...
  if (room()) {
    const rows = await queryRows(
      `SELECT id, received_at, raw_payload, payload_sha256, http_metadata, error
        ...
        ORDER BY received_at, id LIMIT $2`, [cutoff, room()],
    );
    for (const r of rows) {
      const age = utc(r.received_at);
      const content = { raw_payload: r.raw_payload, http_metadata: r.http_metadata, error: r.error };
      out.push({ sourceTable: "clean.adb_ingest_events", recordId: `adb_ingest_events:${r.id}:provider_content`, contentClass: "webhook_raw_delivery", contentColumns: ["raw_payload","http_metadata","error"], ageTimestampUtc: age, expiresAtUtc: expiryFromAge(age, retentionDays), contentHash: sha256(content), payloadHash: r.payload_sha256 || undefined, kind: "adb_ingest_events", key: Number(r.id) });
    }
  }
  ...
  if (room()) {
    const rows = await queryRows(
      `SELECT population_query_id, raw_persisted_at_utc, raw_payload, response_hash
        ...
        ORDER BY raw_persisted_at_utc, population_query_id LIMIT $2`, [cutoff, room()],
    );
    for (const r of rows) {
      const age = utc(r.raw_persisted_at_utc);
      out.push({ sourceTable: "clean.fids_query_response", recordId: `fids_query_response:${r.population_query_id}:raw_payload`, contentClass: "fids_population", contentColumns: ["raw_payload"], ageTimestampUtc: age, expiresAtUtc: expiryFromAge(age, retentionDays), contentHash: sha256(r.raw_payload), payloadHash: r.response_hash || undefined, kind: "fids_query_response", key: String(r.population_query_id) });
    }
  }
  ...
  if (room()) {
    ...
    if (exists.rows[0]?.c) {
      ...
      for (const r of rows) {
        const age = utc(r.created_at);
        out.push({ sourceTable: "clean.monitored_flights_v2", recordId: `monitored_flights_v2:${r.id}:raw_api_data`, contentClass: "legacy_provider_raw", contentColumns: ["raw_api_data"], ageTimestampUtc: age, expiresAtUtc: expiryFromAge(age, retentionDays), contentHash: sha256(r.raw_api_data), payloadHash: r.raw_api_sha256 || undefined, kind: "monitored_flights_v2", key: Number(r.id) });
      }
    }
  }

  return out.sort((a, b) => a.ageTimestampUtc.localeCompare(b.ageTimestampUtc) || a.recordId.localeCompare(b.recordId)).slice(0, limit);
}
...
async function insertTombstone(client: PoolClient, c: RetentionExpiryCandidate, runId: string, pHash: string): Promise<void> {
  const inserted = await client.query(
    ...
    [c.recordId, c.contentHash, c.expiresAtUtc, pHash, c.contentClass, c.sourceTable, c.contentColumns, `max-${DEFAULT_MAX_RAW_RETENTION_DAYS}d/provider-content`, runId],
  );
...
export async function runRetentionExpiry(opts: { apply?: boolean; now?: Date; retentionDays?: number; limit?: number } = {}): Promise<RetentionExpiryResult> {
  const apply = Boolean(opts.apply);
  const now = opts.now ?? new Date();
  const retentionDays = opts.retentionDays ?? validateRetentionDays(process.env.V39_RAW_PROVIDER_RETENTION_DAYS);
  const limit = opts.limit ?? DEFAULT_EXPIRY_BATCH_LIMIT;
  if (apply) assertApplyEvidence();
  const candidates = await collectRetentionExpiryCandidates(now, retentionDays, limit);
  ...
  const resultBase = {
    ...
    runId,
    cutoffUtc: new Date(now.getTime() - retentionDays * 86_400_000).toISOString(),
    retentionDays,
    candidates,
    expiredCount,
  };
