...
function buildPickPrompt(picks: PickSummary[], pool: OfferSummary[]): string {
  ...
  return [
    ...
    "",
    "Rules to enforce (compare picks against the FULL OFFER POOL above, not just against each other):",
    ...
    '2. "Fastest" must equal the offer in the pool with the shortest `duration_minutes` overall — UNLESS at least one nonstop (`stops=0`) exists in the pool, in which case the Fastest pick MUST be a nonstop AND must have the shortest `duration_minutes` among all nonstops in the pool. A 1+ stop pick when an untaken nonstop exists in the pool is always a violation, even if the 1-stop has fewer raw minutes.',
