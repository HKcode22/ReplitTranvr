...
-   **Legal Pages:** Dedicated public pages for privacy policy and terms of service. Both carry a "Version 1.0 · Last updated" line with a stable anchor (`#policy-v1`, `#terms-v1`) for future versioning. The registered legal entity is left as a clearly labeled `[LEGAL ENTITY NAME]` placeholder on first mention in each document — swap in the real entity name in one place when finalized; jurisdiction is set to Missouri, USA in Terms §19. Contact addresses (`hello@travnr.com` primary, `support@travnr.com` support) are consistent across both pages and the transactional email footer.
